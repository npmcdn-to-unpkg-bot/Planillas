...
angular
  ...
  .run(function ($rootScope, $location, $log, $http2, URLS,editableOptions, editableThemes,$modal) {
    ...
    function redirects(){
      ...
      switch (locationPath) {
        ...
        case "/GenerarPlanilla":
         // $location.url('/GenerarPlanilla');
          if($rootScope.GF.isSecre()){$location.url("/Gestion");}
        ...
        case "/CloseSystem":
          ...
          break;
        case "/queryBuild":
          if(!$rootScope.GF.isRoot()){$location.url("/Gestion");}
    ...
    $rootScope.GF.load_unidades_academicas = function (fcnScs, fcnErr,items) {
      if($rootScope.GLOBALS.UNIDADES_ACADEMICAS){fcnScs&&fcnScs();return;}
